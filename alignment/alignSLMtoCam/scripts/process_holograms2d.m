...
excludeTrials = false(1, size(slmXYZ, 2));
...
modelterms =unique([0 0; 1 0; 0 1; 0 1;...
    1 1; 1 0; 0 1 ; 1 1 ;...
    2 0; 0 2; 0 0;  ...
    2 0; 2 1; 0 2; 1 2; 0 1;  1 0; ... ];  %XY spatial calibration model for Power interpolations
    2 2; 2 0; 0 2; 2 1; 1 2; 1 1;], 'rows');
...
refAsk = (slmXYZ(1:2,1:end-holdback))';
refGet = (basXYZ(1:2,1:end-holdback))';
...
clf
ax = gca();
[SLMtoCam, trialN] = function_2DCoCIterative(refAsk,refGet,modelterms,errScalar,0,ax);
title('SLM to Cam v1')

Ask = refAsk;
True = refGet;
Get = function_Eval2DCoC(SLMtoCam,Ask);

figure(103);clf
subplot(1,2,1)
scatter(True(:,1),True(:,2),'*','k')
hold on
scatter(Get(:,1), Get(:,2),'o','r')
...
subplot(1,2,2)
scatter(True(:,1),True(:,2),[],refRMS,'filled');
colorbar
ylabel('Y Axis Pixels')
xlabel('X axis Pixels')
zlabel('Depth \mum')
...
Ask = (slmXYZ(1:2,end-holdback:end))';
True = (basXYZ(1:2,end-holdback:end))';
Get = function_Eval2DCoC(SLMtoCam,Ask);
...
disp('Mean:')
disp(['X: ' num2str(mean(xErr)/pxPerMu) 'um. Y: ' num2str(mean(yErr)/pxPerMu) 'um.']);
disp('Max:')
disp(['X: ' num2str(max(xErr)/pxPerMu) 'um. Y: ' num2str(max(yErr)/pxPerMu) 'um.']);

subplot(1,3,2)
scatter(True(:,1),True(:,2),[],RMS,'filled');
colorbar
ylabel('Y Axis Pixels')
xlabel('X axis Pixels')
zlabel('Depth \mum')
...
refAsk = (basXYZ(1:2,1:end-holdback))';
refGet = (slmXYZ(1:2,1:end-holdback))';

camToSLM = function_2DCoC(refAsk,refGet,modelterms);

figure(1401);
clf
ax = gca();
[camToSLM, trialN] = function_2DCoCIterative(refAsk,refGet,modelterms,errScalar,0,ax);
title('cam to slm v1')

Ask = (basXYZ(1:2,end-holdback:end))';
True = (slmXYZ(1:2,end-holdback:end))';
Get = function_Eval2DCoC(camToSLM,Ask);
...
rtXYZ = function_Eval2DCoC(SLMtoCam,function_Eval2DCoC(camToSLM,basXYZ(1:2,end-holdback:end)'));

err = sqrt(sum((rtXYZ - basXYZ(1:2,end-holdback:end)').^2,2));
...
figure(1);clf
subplot(2,3,1)
scatter(slmXYZ(1,:),slmXYZ(2,:),[],intVal,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
...
subplot(2,3,2)
scatter(slmXYZ(1,:),slmXYZ(2,:),[],polyvaln(SLMtoPower,slmXYZ(1:2,:)'),'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
...
scatter(slmXYZ(1,:),slmXYZ(2,:),[],basVal,'filled');
...
subplot(2,3,5)
c = sqrt((polyvaln(SLMtoPower,slmXYZ(1:2,:)')-intVal').^2);
scatter(slmXYZ(1,:),slmXYZ(2,:),[],c,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
title('Error RMS (A.U.)')
...
subplot(2,3,3)
c = (polyvaln(SLMtoPower,slmXYZ(1:2,:)').^2);
scatter(slmXYZ(1,:),slmXYZ(2,:),[],c,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
...
c = (polyvaln(SLMtoPower,slmXYZ(1:2,:)').^2)-normVal';
scatter(slmXYZ(1,:),slmXYZ(2,:),[],c,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
...
% bas2shoot = [rX rY];
% 
% testSLM = function_Eval2DCoC(camToSLM, bas2shoot);
% expectBas = function_Eval2DCoC(SLMtoCam, testSLM);
% 
% testSLM(:, 3) = slmZrange(1);
...
%             end
% 
%             targs_this_plane = 1:size(expectBas, 1);
%             % choose rand holos
...
%         hold on
%         subplot(1,2,1)
%         scatter(bas_coords{i}{j}(:,1),bas_coords{i}{j}(:,2), [], cmap(c,:), 'filled')%, 'MarkerFaceAlpha',0.7)
%         hold on
%         title('Bas Coords')
%         subplot(1,2,2)
%         scatter(slm_coords{i}{j}(:,1),slm_coords{i}{j}(:,2), [], cmap(c,:), 'filled')%, 'MarkerFaceAlpha',0.7)
%         title('SLM Coords')
%     end
% end
% %% compute holos
...
%     meanz = 0;%mean(cellfun(@(x) mean(x(:,3)),bas_coords{i}));
...
% holdback = 30;%50
% 
% refAsk = (slmXYZ4(1:2,1:end-holdback))';
% refGet = (basXYZ4(1:2,1:end-holdback))';
% 
% %  SLMtoCam = function_3DCoC(refAsk,refGet,modelterms);
% 
% errScalar = 3; %2.8;%2.5;
% figure(1977);clf;subplot(1,2,1)
% [SLMtoCam, trialN] = function_2DCoCIterative(refAsk,refGet,modelterms,errScalar,0);
% title('SLM to Cam v2')
% 
% Ask = refAsk;
% True = refGet;
...
% 
% 
% Ask = (slmXYZ4(1:2,end-holdback:end))';
% True = (basXYZ4(1:2,end-holdback:end))';
% Get = function_Eval2DCoC(SLMtoCam,Ask);
% 
% figure(101);clf
% subplot(1,2,1)
...
% 
% 
% refAsk = (basXYZ4(1:2,1:end-holdback))';
% refGet = (slmXYZ4(1:2,1:end-holdback))';
% 
% %  camToSLM = function_3DCoC(refAsk,refGet,modelterms);
% 
% subplot(1,2,2)
% [camToSLM, trialN] = function_2DCoCIterative(refAsk,refGet,modelterms,errScalar,0);
% title('Cam to SLM v2')
% 
% Ask = (basXYZ4(1:2,end-holdback:end))';
% True = (slmXYZ4(1:2,end-holdback:end))';
% Get = function_Eval2DCoC(camToSLM,Ask);
% 
% 
% figure(101);
...
% out.CoCmodelterms = modelterms;
% 
% rtXYZ = function_Eval2DCoC(SLMtoCam,function_Eval2DCoC(camToSLM,basXYZ4(1:2,end-holdback:end)'));
% 
% err = sqrt(sum((rtXYZ - basXYZ4(1:2,end-holdback:end)').^2,2));
% meanRTerr = nanmean(err);
% disp(['The Mean Round Trip RMS error: ' num2str(meanRTerr) ' pixels (' num2str(meanRTerr/pxPerMu) ' um) camera to SLM to camera']);
...
figure(1);clf
subplot(2,3,1)
scatter(slmXYZ4(1,:),slmXYZ4(2,:),[],intVal,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
title('Measured Power (converted to 1p)')
...
subplot(2,3,2)
scatter(slmXYZ4(1,:),slmXYZ4(2,:),[],polyvaln(SLMtoPower,slmXYZ4(1:2,:)'),'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
title('Estimated Power Norm.')
...
subplot(2,3,4)
% scatter3(slmXYZ(1,:),slmXYZ(2,:),slmXYZ(3,:),[],polyvaln(SLMtoPower,slmXYZ(1:3,:)')-intVal','filled');
scatter(slmXYZ4(1,:),slmXYZ4(2,:),[],basVal4,'filled');
...
subplot(2,3,5)
c = sqrt((polyvaln(SLMtoPower,slmXYZ4(1:2,:)')-intVal').^2);
scatter(slmXYZ4(1,:),slmXYZ4(2,:),[],c,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
title('Error RMS (A.U.)')
...
subplot(2,3,3)
c = (polyvaln(SLMtoPower,slmXYZ4(1:2,:)').^2);
scatter(slmXYZ4(1,:),slmXYZ4(2,:),[],c,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
...
c = (polyvaln(SLMtoPower,slmXYZ4(1:2,:)').^2)-normVal';
scatter(slmXYZ4(1,:),slmXYZ4(2,:),[],c,'filled');
ylabel('Y Axis SLM units')
xlabel('X axis SLM units')
zlabel('Z axis SLM units')
...
subplot(1,2,2);
scatter(slmXYZ(1,:),slmXYZ(2,:),  [],FWHM,'filled')
