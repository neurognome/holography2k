function [CoC, trialN] = function_2DCoCIterative(refAsk,refGet,modelterms,errScalar,Verbose,axisHandle)
if nargin<6
    figure(321)
    clf
    axisHandle = gca;
...
while go
    ...
    trialN(errs)=[];
    c=c+1;
    CoC = function_2DCoC(refAsk,refGet,modelterms);
    Get = function_Eval2DCoC(CoC,refAsk);
...
subplot(axisHandle)
scatter(refGet(:,1),refGet(:,2),'o')
hold on
scatter(Get(:,1),Get(:,2),'*','r')
